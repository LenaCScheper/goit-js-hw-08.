...
"use strict";var e=t(require("lodash/throttle"));function t(e){return e&&e.__esModule?e:{default:e}}const a=document.querySelector("input[type=email]"),o=document.querySelector("textarea[name=message]"),s=document.querySelector("button[type=submit]"),l={email:"",message:""},r=localStorage.getItem("feedback-form-state");function n(){l.email=a.value,localStorage.setItem("feedback-form-state",JSON.stringify(l))}function u(){l.message=o.value,localStorage.setItem("feedback-form-state",JSON.stringify(l))}function m(e){e.preventDefault();const t=JSON.parse(localStorage.getItem("feedback-form-state"));console.log("user info submited: ",t),localStorage.removeItem("feedback-form-state"),a.value="",o.value=""}function c(){r&&(a.value=JSON.parse(r).email,o.value=JSON.parse(r).message,console.log(`в хранилище осталось email = ${JSON.parse(r).email} и message = ${JSON.parse(r).message}`))}a.addEventListener("input",(0,e.default)(n,500)),o.addEventListener("input",(0,e.default)(u,500)),s.addEventListener("click",m),c();
},{"lodash/throttle":"Ywd4"}]},{},["TTyb"], null)
//# sourceMappingURL=/goit-js-hw-08./03-feedback.8df41cad.js.map